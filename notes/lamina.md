...
- [THREE.BackSide](https://threejs.org/docs/#api/en/materials/Material) indicates that the backside of the geometry's faces will cast shadows. This is used in the example above because the subject is within the environment's mesh.
...
existed at commit `523b32c`, [url](https://github.com/pmndrs/lamina/blob/523b32c57ecf286bf7d5c7341b7f83b5f1152361/src/index.tsx)

- `Base` was renamed to `Color` in commit `445ff4a`, [url](https://github.com/pmndrs/lamina/commit/3e0ab3c2bceff77bde77e3109e1e5c4c10561a09)

## `LayerMaterial` source code
...
this.uniforms = {
  u_lamina_color: {
    value:
      typeof _baseColor === "string"
        ? new THREE.Color(_baseColor).convertSRGBToLinear()
        : _baseColor,
  },
  u_lamina_alpha: {
    value: _alpha,
...
- `.convertSRGBToLinear` converts the color from sRGB to linear so that it can be used in a shader.
- To build a layer material's shader:
  1. iterate through all the layers
  2. for each layer, get the vertex and fragment, variables and shader
  3. append each
...
_final vertex shader of layer material_
...
_final fragment shader of layer material_
